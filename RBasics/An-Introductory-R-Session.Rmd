...
This tutorial is available in [HTML](An-Introductory-R-Session.html) format for screen reading and [PDF](An-Introductory-R-Session.pdf) for printing.
...
It is also possible to define user-defined classes.
...
```
So it is necessary to convert the character to numeric first
...
A matrix is a two-dimensional data structure that can be used for matrix algebra. To create a matrix, we can first create a one-dimensional vector and then reformat it as two-dimensional matrix with `nrow` rows and `ncol`columns:
...
Vectors, matrices and arrays have an important limitation: they can only contain one data type (mode), either numeric or character. So, if a single element is of type character, the whole matrix will be of mode character.
...
Lists are extremely powerful und flexible and may be discussed later.
