...
leaves %>% group_by(group) %>%
  summarize(mean=mean(length), sd=sd(length), min=min(length), max=max(length))
```
...
```{r}
dat2 <-  pivot_longer(leaves, c("length", "width", "stalk"))
dat3 <-  group_by(dat2, group, name)
dat4 <-  summarize(dat3, mean=mean(value), sd=sd(value),
